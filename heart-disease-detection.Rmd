...
performance <- function(pred.prob, pred.class, method, test, positive){
  ...
  Sensitivity <- con$byClass[2]
  Specificity <- con$byClass[1]
...
```{r, echo=FALSE, fig.height = 5, fig.width = 8, fig.align = "center"}
#dig into the person who has the desease
...
```{r}
performance(nb_prob, as.factor(nb_pred),
...
plot(glcv)
sl <- recordPlot()
png(file = paste0("lambda",".png"), height = 6, width = 6, units = 'in', res = 300)
plot.new()
sl
dev.off()
```
...
xgbcv_test_auc <- xgbcv$evaluation_log$test_auc_mean[xgbcv$best_ntreelimit]
```
...
```{r, echo=FALSE}
fact_col <- colnames(data)[sapply(data, is.factor)]
...
```{r}
# create tasks
train_task <- makeClassifTask(data=train_data, target='target')
...
params <- makeParamSet(makeNumericParam("eta", lower=0.01, upper=0.3),      
                       ...
                       makeIntegerParam("max_depth", lower=3L, upper=22L),
...
```{r}
xgb_model <- xgb.train(params=lrn_tune$par.vals,
                       data=dtrain,
                       nrounds=500,
                       print_every_n=10,
                       watchlist=list(val=dtest, train=dtrain),
...
```{r}
confusionMatrix(factor(xgb_pred), factor(te_target))
...
```{r}
xgb.plot.shap(data=tr_data,
              model=xgb_model,
              features=names(shap_values$mean_shap_score),
              n_col=3,
              plot_loess=T)
```
